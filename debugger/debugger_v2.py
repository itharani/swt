...
def scan(folder_path, file_extension="*.py"):
    """Read code content with line numbers for better debugging context."""
    code_files = []
...
def send_error_to_gpt(tracebacks):
    ...
    # print(content[-1], "file")
    # content = json.loads(content)
    # print(type(content), "content_type")
    # print(content[-1], "content[-1]")
    # apply_changes(content[-1], content, confirm=True)
...
def apply_changes(changes_by_file: Dict[str, Dict], confirm: bool = False):
  ...
  # Initially assume it didn't solve the issue
